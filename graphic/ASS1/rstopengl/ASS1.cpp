...
bool leftGiftOpen = false;
bool rightGiftOpen = false;
void keyboard(unsigned char key, int x, int y) {
	float moveAmount = 0.05;
	if ((key == 'd' || key == 'D') && personX <= 0.775) {
		personX += moveAmount;
	}
	...
	if (scene == 2) {
		if ((key == 'q' || key == 'Q') && leftGiftOpen == false) {
			leftGiftOpen = !leftGiftOpen;
		}
		if ((key == 'e' || key == 'E') && rightGiftOpen == false) {
			rightGiftOpen = !rightGiftOpen;
		}
		glutPostRedisplay();
	}
}
...
void createMenu() {
	...
	glutAddMenuEntry("Switch to scene 1", 1);
	glutAddMenuEntry("Switch to scene 2", 2);
...
void drawNewScene() {
	...
	const char* messageLine55 = "Press E to open gift";
	for (const char* c = messageLine55; *c != '\0'; c++) {
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *c);
	}

	glRasterPos2f(-0.55, -0.9);
	const char* messageLine66 = "Press Q to open gift";
	for (const char* c = messageLine66; *c != '\0'; c++) {
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *c);
	}
